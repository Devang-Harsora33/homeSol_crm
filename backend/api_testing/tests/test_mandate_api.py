...
REPORT_FILE = "mandate_api_test_report.csv"
MODULE_NAME = "Mandate"
...
def randomize_value(key, value):
    ...
    if "email" in key.lower():
        return f"mandate_test_{timestamp}_{random.randint(100, 999)}@example.com"
    ...
    for item in items:
        ...
        if module_name.lower() == name.lower() or module_name.lower() in parent_name.lower():
            if "item" in item:
                ...
                # Special handling for "localhost" in Mandate APIs identified in the collection
                if "localhost" in url_raw:
                    url = url_raw.replace("http://localhost:8002", BASE_URL)
                else:
