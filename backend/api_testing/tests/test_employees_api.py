...
REPORT_FILE = "employees_api_test_report.csv"
MODULE_NAME = "Employees" # Top-level folder name
...
def randomize_value(key, value):
    ...
    if "email" in key.lower():
        return f"emp_test_{timestamp}_{random.randint(100, 999)}@example.com"
    if any(k in key.lower() for k in ["mobile", "phone", "whatsapp", "device_id"]):
        return f"9{random.randint(100000000, 999999999)}" if "device_id" not in key.lower() else f"device-{timestamp}-{random.randint(1000, 9999)}"
    if "date" in key.lower() and isinstance(value, str) and re.match(r"\d{4}-\d{2}-\d{2}", value):
        return datetime.now().strftime("%Y-%m-%d")
    
    # Specific Frappe IDs that might need randomization if they are created by the API
    if "leave_type_name" in key.lower():
        return f"TestLeaveType-{timestamp}"
    
    # Specific values that might need to be dynamic or valid
    if "log_type" in key.lower(): # For Check In, ensure it's valid
...
def extract_module_requests(items, module_name, parent_name=""):
    ...
    for item in items:
        ...
        # Check if we are inside the target module's hierarchy
        # This allows capturing subfolders like "My Profile" within "Employees"
        if module_name == name or module_name in parent_name:
            if "item" in item: # It's a folder, recurse
                ...
                if "localhost" in url_raw:
                    # Replace localhost with BASE_URL, assuming 8000 or 8002
        ...
        # If not yet in the target module's hierarchy, keep searching top-level items for the module folder
        elif "item" in item: 
            extracted.extend(extract_module_requests(item["item"], module_name, current_full_name))
...
def run_tests():
    ...
    for req in unique_requests:
        ...
        for role, token in TOKENS.items():
            ...
            # Apply dynamic body randomization for POST/PUT requests
            test_body = None
            if req["method"] in ["POST", "PUT"] and isinstance(req.get('body'), dict):
                test_body = process_body_recursive(req.get('body'))
            elif req.get('body'): # For other methods or non-dict bodies, use as is
