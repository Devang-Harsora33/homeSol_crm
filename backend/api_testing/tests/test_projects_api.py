...
REPORT_FILE = "projects_api_test_report.csv"
MODULE_NAME = "Property Projects"
...
def extract_module_requests(items, module_name, parent_name=""):
    """
    Extracts only requests that belong to a specific folder/module.
    """
    extracted = []
    for item in items:
        name = item.get("name", "")
        # Check if the current item's name matches the module name OR if we are already inside the module folder
        is_in_module_path = module_name in name or (parent_name and module_name in parent_name)
        
        if is_in_module_path:
            if "item" in item: # It's a folder
                current_name = f"{parent_name} > {name}" if parent_name else name
                extracted.extend(extract_module_requests(item["item"], module_name, current_name))
        ...
        # If not in the module path yet, keep searching recursively
        elif "item" in item:
            extracted.extend(extract_module_requests(item["item"], module_name, name))
                
    return extracted

# =================================================================
# 3. PROJECTS TESTING LOGIC
# =================================================================
        ...
        for role, token in TOKENS.items():
            ...
            try:
                if req["method"] in ["POST", "PUT"] and isinstance(req.get('body'), dict):
                    response = requests.request(req["method"], req["url"], headers=test_headers, json=req.get('body'), timeout=20)
                else:
                    response = requests.request(req["method"], req["url"], headers=test_headers, data=req.get('body'), timeout=20)
