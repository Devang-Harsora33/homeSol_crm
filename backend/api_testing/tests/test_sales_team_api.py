...
REPORT_FILE = "sales_team_api_test_report.csv"
...
def randomize_value(key, value):
    ...
    if "email" in key.lower():
        return f"st_test_{timestamp}_{random.randint(100, 999)}@example.com"
    ...
    for item in items:
        ...
        if module_name == name or module_name in parent_name:
            if "item" in item:
                ...
                # Special handling for "localhost" in some URLs from the collection
