...
# Update these with your actual API Tokens
TOKENS = {
...
REPORT_FILE = "leads_api_test_report.csv"
...
def randomize_value(key, value):
    ...
    if "email" in key.lower():
        return f"lead_test_{timestamp}_{random.randint(100, 999)}@example.com"
...
def extract_leads_requests(items, parent_name=""):
    """
    Extracts only requests that belong to the 'Leads' folder or subfolders.
    """
    extracted = []
    for item in items:
        name = item.get("name", "")
        ...
        # If we are in the Leads folder or its children, extract requests
        if "Leads" in full_name or parent_name == "Leads":
            if "item" in item:
                extracted.extend(extract_leads_requests(item["item"], full_name))
            elif "request" in item:
                req = item["request"]
                url_raw = req["url"]["raw"] if isinstance(req["url"], dict) else req["url"]
                url = url_raw.replace("{{baseurl_production}}", BASE_URL)
                
                # Cleanup localhost if present
                if "localhost" in url:
            ...
            # If not in Leads, check if this folder IS the Leads folder
            if "item" in item:
                extracted.extend(extract_leads_requests(item["item"], full_name))
...
# =================================================================
# 3. LEADS TESTING LOGIC
# =================================================================

def run_leads_tests():
    print(f"🎯 Starting LEADS Module Testing")
    ...
    # Extracting only Lead related requests
    all_requests = extract_leads_requests(collection["item"])
    
    # Deduplicate by URL and Method to avoid re-testing the same thing if structure is messy
    unique_requests = []
    seen = set()
    ...
    print(f"✅ Found {len(unique_requests)} unique Lead endpoints.")
    ...
    for req in unique_requests:
        ...
        for role, token in TOKENS.items():
            ...
            try:
                ...
                # 2xx: Pass
                # 403: Forbidden (Might be expected if role shouldn't have access)
                # 401: Unauthorized
                # 5xx: Server Error
    ...
    if report_data:
        ...
        print(f"\n📊 Leads report saved to: {REPORT_FILE}")
