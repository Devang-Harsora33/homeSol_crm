...
REPORT_FILE = "site_visits_api_test_report.csv"
...
def randomize_value(key, value):
    ...
    if "email" in key.lower():
        return f"visit_test_{timestamp}_{random.randint(100, 999)}@example.com"
...
def extract_module_requests(items, module_name, parent_name=""):
    ...
    for item in items:
        ...
        if module_name in full_name or parent_name == module_name:
            if "item" in item:
                extracted.extend(extract_module_requests(item["item"], module_name, full_name))
...
# =================================================================
# 3. SITE VISITS TESTING LOGIC
# =================================================================

def run_tests():
    print(f"🎯 Starting SITE VISITS Module Testing")
    ...
    all_requests = extract_module_requests(collection["item"], "Site Visits")
    ...
    print(f"✅ Found {len(unique_requests)} unique Site Visit endpoints.")
    ...
    if report_data:
        ...
        print(f"\n📊 Site Visits report saved to: {REPORT_FILE}")
